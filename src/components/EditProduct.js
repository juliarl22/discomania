...
export default function Product() {
    ...
    const [datosProducto, setDatosProducto] = useState({ idproducto: idproducto, nombre: "", descripcion: "", artistas: "", imagen: "", precio: 0 });
        ...
        const fetchData = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.ok) {
                        setDatosProducto(data.datos[0]);
                        setSelectedValue(data.datos[0].idcategoria);
                    } else {
    ...
    const handleSelectChange = (event) => {
        ...
        setDatosProducto({ ...datosProducto, idcategoria: event.target.value });
    };
    ...
    const handleSubmit = async (event) => {
        ...
        if (productsValidator(data)) {
            ...
            let response = await fetch(config.apiUrl + 'post-update-productos.php', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                },
                body: data
                //JSON.stringify({ idproducto: datosProducto.idproducto, idcategoria: datosProducto.idcategoria, nombre: data.get('nombre'), descripcion: data.get('descripcion'), artistas: data.get('artistas'), imagen: "", precio: data.get('precio') })
            })
            if (response.ok) {
        ...
        <Container component="main" maxWidth="xs">
            ...
            >
                <Typography component="h1" variant="h5">
                    Modificación de datos de un disco
                </Typography>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                            <TextField
                                ...
                                fullWidth
                                name="descripcion"
                                onChange={(e) => setDatosProducto({ ...datosProducto, descripcion: e.target.value })}
                        ...
                        <Grid item xs={12}>
                            <TextField
                                ...
                                fullWidth
                                name="artistas"
                                onChange={(e) => setDatosProducto({ ...datosProducto, artistas: e.target.value })}
                                value={datosProducto.artistas}
                                label="Artistas"
                                id="artistas"
                                multiline
                                rows={2}
                        ...
                        <Grid item xs={12}>
                            <Grid container justifyContent="center">
                                ...
                                />) : (
                                    <img
                                        src={config.imageFolder + idproducto}
                                        width="100"
                                        height="100"
                                        id="imagenActual"
                                        alt="Imagen categoría"
                        ...
                        <Grid item xs={12}>
                            <TextField
                                ...
                                fullWidth
                                name="precio"
                                onChange={(e) => setDatosProducto({ ...datosProducto, precio: e.target.value })}
